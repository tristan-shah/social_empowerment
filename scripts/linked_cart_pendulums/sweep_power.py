...
from soc_emp.empowerment import compute_multiagent_control
from soc_emp.utils import smooth_angle_wrap

## import the empowerment grad computation for linked cart pendulum
from test import compute_multiagent_empowerment_grad, set_anchor_gain
...
def get_linked_cart_pendulum_outcome(traj: Array):
    '''
    ...
    left_angle_from_bottom = jnp.abs(smooth_angle_wrap(traj[:, 1]))
    right_angle_from_bottom = jnp.abs(smooth_angle_wrap(traj[:, 3]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--anchor_gain', type = float, default = 1.0)
    ...
    print(f'Anchor Gain = {args.anchor_gain}')
    ...
    output_dir = Path(f'results/carts/resolution={num_powers}_seed={seed}_anchor-gain={args.anchor_gain}_horizon={horizon}_alpha={alpha}_observation_noise={observation_noise}')
    ...
    dyn = set_anchor_gain(dyn, args.anchor_gain)
    ...
    for batch_idx in range(num_batches):
        ...
        batch_outcomes = batch_get_linked_cart_pendulum_outcome(batch_X)
