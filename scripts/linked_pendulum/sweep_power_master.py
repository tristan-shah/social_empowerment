...
if __name__ == '__main__':
    ...
    parser.add_argument('--agent_0_powers', type = int, default = 50)
    ...
    agent_0_powers = jnp.linspace(0.1, 1.0, 50)
    agent_1_powers = jnp.linspace(2.0, 10.0, 200)
    device_batch_size = args.batch_size
    num_devices = jax.device_count()

    output_dir = Path(f'results/sweep_power_inverted_hinge_left_master/seed={seed}_horizon={horizon}_alpha={alpha}_observation_noise={observation_noise}_stiffness={stiffness}')
    ...
    I, J = jnp.meshgrid(jnp.arange(len(agent_0_powers)), jnp.arange(len(agent_1_powers)), indexing='ij')
    ## all possible combinations of powers
    pairs = jnp.stack([agent_0_powers[I], agent_1_powers[J]], axis=-1).reshape(-1, 2)
