...
if __name__ == '__main__':
    ...
    # results/linked_pendulum/control_type=egoistic/resolution=200-state_type=angle-horizon=125-alpha=0.01-observation_noise=1.0-stiffness=3.0-damping=0.1-steps=2000-min_power=0.1-max_power=4.0/outcomes_batch_0.npy

    root = Path(f'results/linked_pendulum/control_type={control_type}/resolution=200-state_type={state_type}-horizon={horizon}-alpha={alpha}-observation_noise={observation_noise}-stiffness={stiffness}-damping={damping}-steps=2000-min_power=0.1-max_power=4.0')
    ...
    axes.set_xticklabels([f'{v:.1f}' for v in powers[ticks]], rotation=90)
    axes.set_yticks(ticks)
    axes.set_yticklabels([f'{v:.1f}' for v in powers[ticks]])
